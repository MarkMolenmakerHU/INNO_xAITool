...
from UI.Code.Components import TextModal


def bind_selectable(stacked_widget, scroll_contents, select_action):
    def show_description(event):
        TextModal(stacked_widget, "Dit is wat extra info over de dataset.")

    def show_info(event):
        TextModal(stacked_widget, "Dit zijn wat use cases met de dataset.")
